...
static void BuildIndices(const char* feedsFileName, hashset* wordInfo, vector* fixedArticles, hashset* stopWords);
static void ProcessFeed(const char* remoteDocumentName, hashset* wordInfo, vector* fixedArticles, hashset* stopWords);
static void PullAllNewsItems(urlconnection* urlcon, hashset* wordInfo, vector* fixedArticles, hashset* stopWords);
...
static void BuildIndices(const char* feedsFileName, hashset* wordInfo, vector* fixedArticles, hashset* stopWords)
{
	...
	{						   // ignore everything up to the first selicolon of the line
		...
		ProcessFeed(remoteFileName, wordInfo, fixedArticles, stopWords);
	}
...
static void ProcessFeedFromFile(char* fileName, hashset* wordInfo, vector* fixedArticles, hashset* stopWords)
{
	FILE* infile;
	streamtokenizer st;
	...
	ScanArticle(&st, (const char*)fileName, articleDescription, (const char*)fileName, wordInfo, fixedArticles, stopWords);
...
static void ProcessFeed(const char* remoteDocumentName, hashset* wordInfo, vector* fixedArticles, hashset* stopWords)
{
	...
	{
		ProcessFeedFromFile((char*)remoteDocumentName + strlen(filePrefix), wordInfo, fixedArticles, stopWords);
		return;
	}
	...
	case 0:
		...
		break;
	case 200:
		PullAllNewsItems(&urlconn, wordInfo, fixedArticles, stopWords);
		break;
	case 301:
	case 302:
		ProcessFeed(urlconn.newUrl, wordInfo, fixedArticles, stopWords);
		break;
	default:
...
static void PullAllNewsItems(urlconnection* urlconn, hashset* wordInfo, vector* fixedArticles, hashset* stopWords)
{
	streamtokenizer st;
	...
	{ // if true is returned, then assume that <item ...> has just been read and pulled from the data stream
		ProcessSingleNewsItem(&st, wordInfo, fixedArticles, stopWords);
	}
...
static void ProcessSingleNewsItem(streamtokenizer* st, hashset* wordInfo, vector* fixedArticles, hashset* stopWords)
{
	...
	ParseArticle(articleTitle, articleDescription, articleURL, wordInfo, fixedArticles, stopWords);
}
													...
													hashset* wordInfo, vector* fixedArticles, hashset* stopWords)
{
	...
	case 200:
		...
		STNew(&st, urlconn.dataStream, kTextDelimiters, false);
		ScanArticle(&st, articleTitle, u.serverName, articleURL, wordInfo, fixedArticles, stopWords);
		STDispose(&st);
		break;
	case 301:
	case 302: // just pretend we have the redirected URL all along, though index using the new URL and not the old one...
		ParseArticle(articleTitle, articleDescription, urlconn.newUrl, wordInfo, fixedArticles, stopWords);
		break;
	default:
