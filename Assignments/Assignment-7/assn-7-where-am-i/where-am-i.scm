...
(define (partition pivot num-list cmpFunction)
  (if (null? num-list) '(() ())
      (let ((split-of-rest (partition pivot (cdr num-list) cmpFunction)))
	(if (equal? (cmpFunction (car num-list) pivot) < )
...
(define (quicksort num-list cmpFunction)
  (if (<= (length num-list) 1) num-list
      (let ((split (partition (car num-list) (cdr num-list) cmpFunction)))
	(append (quicksort (car split) cmpFunction)
		(list (car num-list)) 
		(quicksort (cadr split) cmpFunction)))))
...
;; This function finds all intersection points 
;; Of every two circle
(define (intersection-points circles)
    (if (< (length circles) 2) '()
        (append (apply append (map (lambda (circle)
...
;; This function finds product
;; Of distances from single point to every other
(define (distance-product point otherPoints)
  (apply * (map (lambda (curPoint)
                  (if (equal? point curPoint) 1
                    (dist point curPoint)
                  )
                )
                otherPoints)
...
;; This function finds distance-product
;; For every point
(define (rate-points points)
  (map (lambda (point)
    (cons (distance-product point points) (list point))
    ) points)
)
...
;; This function is compare function
;; For points
(define (cmpFunction point1 point2)
...
;; This function sorts points 
;; Ascending order
(define (sort-points points)
  (quicksort points cmpFunction)
)
...
;; This function evaluates to first half
;; Of sorted points
(define (clumped-points points)
  (map  (lambda (pointWithRate)
      (car (cdr pointWithRate))
      ) ((lambda (sortedPoints)
          (prefix-of-list sortedPoints (quotient (length sortedPoints) 2))
...
;; This function finds average point
;; And distance product of it to every points
(define (average-point points)
  ((lambda (avg) (cons (distance-product avg points) (list avg))
  ) (make-pt (/ (apply + (map (lambda (curPoint) (x curPoint)) points)) (length points)) (/ (apply + (map (lambda (curPoint) (y curPoint)) points)) (length points))))
)
...
(define (where-am-i distances starLocations)
  (quicksort (map best-estimate            
            (all-guesses distances starLocations)
          ) cmpFunction)
)
